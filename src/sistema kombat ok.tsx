...
export default function App() {
    ...
    // Form States
    const [customerForm, setCustomerForm] = useState({ name: '', cpf: '', cnpj: '', whatsapp: '', address: '', neighborhood: '', city: '', zip_code: '', credit_limit: 0 });
    ...
    const handleAddMechanic = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert(`Erro ao salvar mecânico: ${error.message || 'Erro desconhecido'}`);
        }
    };
        ...
        try {
            ...
            if (salesData) {
                setStats({
                    ...
                    topProducts: [] // Logic for top products can be added here
                });
            }
    ...
    const handleCompleteOS = async () => {
        ...
        try {
            ...
            if (motorcycle && osForm.km) {
                await supabase.from('motorcycles').update({ current_km: parseInt(osForm.km.toString().replace(',', '.')) || 0 }).eq('id', motorcycle.id);
            }
    ...
    const handleCloseCash = async () => {
        ...
        try {
            const { error } = await supabase.from('cash_sessions').update({
                ...
                closing_balance: parseFloat(closingBalance.toString().replace(',', '.')) || 0,
    ...
    const handleCreateOrder = async (e: React.FormEvent) => {
        ...
        try {
            const { error: orderError } = await supabase.from('purchase_orders').insert([{
                ...
                distributor_name: newOrder.distributor_name,
    ...
    const handleAddCustomer = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const { name, cpf, cnpj, whatsapp, address, neighborhood, city, zip_code } = customerForm;
            const dataToSave = {
                name,
                cpf,
                cnpj,
            ...
            setCustomerForm({ name: '', cpf: '', cnpj: '', whatsapp: '', address: '', neighborhood: '', city: '', zip_code: '', credit_limit: 0 });
            fetchData();
        } catch (error) {
        ...
        try {
            if (editingMotorcycle) {
                const { error } = await supabase.from('motorcycles').update({
                    ...
                    current_km: parseInt(motorcycleForm.current_km.toString().replace(',', '.')) || 0
            ...
            <main className="flex-1 p-8 overflow-y-auto ml-80">
                ...
                >
                    {selectedSaleForReceipt && (
                        <div id="receipt-content" className="bg-white p-2 font-mono text-[11px] leading-tight text-black max-w-[320px] mx-auto overflow-visible print:p-0">
                            <style>{`
                @media print {
                  @page {
                    margin: 0;
              ...
              `}</style>
                            ...
                            <div style={{ textAlign: 'center', marginBottom: '8px' }}>
                                <h4 style={{ fontWeight: 'bold', fontSize: '14px', margin: '0' }}>KOMBAT MOTO PECAS</h4>
                            ...
                            <div style={{ padding: '4px 0' }}>
                                {selectedSaleForReceipt.moto_details ? (
                                    <div style={{ fontSize: '10px' }}>
                                        <p style={{ fontWeight: 'bold' }}>Placa: {selectedSaleForReceipt.moto_details.match(/\((.*?)\)/)?.[1] || '-'}</p>
                            ...
                            <div style={{ marginTop: '16px', paddingTop: '4px', borderTop: '1px dashed black' }}>
                                <p style={{ fontWeight: '900', textAlign: 'center', marginBottom: '4px', textTransform: 'uppercase' }}>TODO DIA 5 DO MES</p>
                                <table style={{ width: '100%', fontSize: '9px', fontWeight: 'bold', borderBottom: '1px solid black', marginBottom: '4px' }}>
                                    <thead>
                                        <tr>
                            ...
                            <div style={{ textAlign: 'center', marginTop: '24px', paddingTop: '8px', borderTop: '1px dashed black' }}>
