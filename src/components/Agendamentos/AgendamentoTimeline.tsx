...
import { CheckCircle2, Clock, PlayCircle, Truck, Package, Wrench, XCircle, ArrowRight } from 'lucide-react';
...
export const AgendamentoTimeline: React.FC<TimelineProps> = ({ agendamentoId }) => {
  ...
  if (history.length === 0) return <div className="text-center py-4 text-sm text-slate-500">Nenhum histórico encontrado.</div>;
        ...
        <div key={item.id} className="flex gap-4">
          ...
          <div className="pb-4">
            <p className="text-[10px] font-black uppercase text-slate-400">
              {format(new Date(item.created_at), "dd/MM/yyyy 'às' HH:mm")} • {item.user_name}
            </p>
